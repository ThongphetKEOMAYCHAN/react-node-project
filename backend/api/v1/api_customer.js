...
router.get("/customer/list", jwt.verify, function (req, res) {
  let sql =
    "SELECT `id`, `uuid`, `name`, `last_name`, `address`, `tel`, `email`, `image`, `create_date` FROM `tbl_customer` ORDER BY create_date DESC";
...
router.post("/customer/delete", function (req, res) {
  ...
  db.query(sql, [uuid], function (err, rs) {
    if (err) throw err;
    res.json({ status: "success", message: "DELETE_SUCCESS" });
  });
});
