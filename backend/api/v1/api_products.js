...
router.post("/create/products", jwt.verify, (req, res) => {
     ...
     db.query(sql, [proName], (err, result) => {
          if (result.length > 0) {
               ...
               const sql = "INSERT INTO tbl_products(proName,price, dateTime, categoryId) values (?,?,?,?)";
               db.query(sql, [proName,price, dateTime, proTypeId], (error, row) => {
...
router.put("/update/products", jwt.verify, (req, res) => {
     ...
     const sql = "UPDATE tbl_products SET proName = ?,price = ?,dateTime = ?, categoryId = ? WHERE id = ?";
...
router.delete("/delete/products", jwt.verify, (req, res) => {
     ...
     if (proId) {
          ...
          db.query(sql, [proId], (err, result) => {
               if (err) throw err;
               res.json({ status: 'delete', message: 'products Deleted' });
          })
     } else {
...
router.get("/get/products", jwt.verify, (req, res) => {
     const sql = 'SELECT pro.`id`, `proName`, `price`, `dateTime`, `category_name`\n'+
...
router.get("/fetch/products", jwt.verify, (req, res) => {
     const sql = 'SELECT `id`, `proName` from tbl_products';
     db.query(sql, (err, result) => {
          if (err) throw err;
          res.send(result);
     })
